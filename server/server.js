...
const basePath = '/person/pdl-fullmakt-ui'
...
server.use(/^(?!.*\/(internal|static)\/).*$/, (req, res) => {
  const namespace = req.headers.host.includes('dev.nav') ? 'dev' : (req.headers.host.includes('q0') || req.headers.host.includes('dev-sbs')) ? 'q0' : 'p'
